...
Register is a "mixin" for endowing any base class with a named registry for its
subclasses and a decorator for registering them.
"""
...
class Registrable:
    ...
    def by_name(cls: Type[T], name: str) -> Type[T]:
        if name not in Registrable._registry[cls]:
            raise NotImplementedError("%s is not a registered name for %s" % (name, cls.__name__))
